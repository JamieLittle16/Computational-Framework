...
type NumericSettingKey = keyof LocalValues;
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ settings, onSettingsChange }) => {
    const [localValues, setLocalValues] = useState<LocalValues>({
        ...
        selectionTintStrength: {
            ...
            isDefault: settings.selectionTintStrength === 0.15,
        },
    });
    ...
    const validateInputs = useMemo<Errors>(() => {
        ...
        if (
            localValues.maxEvalDepth.value < 1 ||
            !Number.isInteger(localValues.maxEvalDepth.value)
        ) {
        ...
        if (
            localValues.selectionTintStrength.value < 0 ||
            localValues.selectionTintStrength.value > 1
        ) {
    ...
    const handleInputChange = useCallback(
        (e: React.ChangeEvent<HTMLInputElement>, id: NumericSettingKey, defaultValue: number) => {
    ...
    const handleBlur = useCallback(
        (id: NumericSettingKey) => {
            if ((errors as unknown as Record<string, string>)[id] === '') {
                onSettingsChange((prev) => ({ ...prev, [id]: localValues[id].value }));
            }
        },
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <Input
                    ...
                    placeholder="100"
                    value={
                        localValues.maxEvalDepth.isDefault
                            ? ''
                            : String(localValues.maxEvalDepth.value)
                    }
                    onFocus={(e) => {
                ...
                />
                {errors.maxEvalDepth && (
                    <p className="text-sm text-red-500">{errors.maxEvalDepth}</p>
                )}
                <p className="text-sm text-gray-500">
                    Maximum depth of evaluation to prevent infinite loops
                </p>
            </div>
            ...
            <div className="space-y-2">
                ...
                <Input
                    ...
                    placeholder="0.15"
                    value={
                        localValues.selectionTintStrength.isDefault
                            ? ''
                            : String(localValues.selectionTintStrength.value)
                    }
                    onFocus={(e) => {
                ...
                />
                {errors.selectionTintStrength && (
                    <p className="text-sm text-red-500">{errors.selectionTintStrength}</p>
                )}
                <p className="text-sm text-gray-500">
                    Strength of the blue tint when node is selected (0 to 1)
                </p>
            </div>
            ...
            <div className="flex items-center gap-2">
                <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                        onSettingsChange((prev) => ({ ...prev, colorMode: checked as boolean }))
                    }
                    disabled={hasErrors}
                />
                <label htmlFor="colorMode" className="text-sm text-gray-700 cursor-pointer">
